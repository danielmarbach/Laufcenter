	...
	'menus' => array(
		...
		array(
			...
			'controls' => array(
				...
				array(
					'name' => 'excerpt_text',
					'label' => __('Read more link text', 'appic'),
					'type' => 'textbox',
					'default' => 'More',
				),
				array(
					'name' => 'is_excerpt',
					'label' => __('Excerpt', 'appcorner'),
					'description' => __('Turn on to automatically shorten the posts on the blog page', 'appic'),
					'type' => 'toggle',
				),
				array(
...
function _getFontSelectionFields($prefix, $defaults = array())
{
	...
	/*$result[] = array(
		'type' => 'select',
		'name' => $prefix . '_font_subset',
		'label' => __('Font Subset', 'appic'),
		'items' => array(
			'data' => array(
				array(
					'source' => 'binding',
					'field' => $faceFieldName,
					'value' => 'cached_vp_get_gwf_subset',
				),
			),
		),
		'default' => $defaults['subset'],
	);*/
...
function _getHomeCallBlocks($length = 3, $additionalOptions = array())
{
	...
	for($i=1; $i<=$length; $i++) {
		$fields = array();
		$dep = array(
			'field' => 'home_ca_title_' .$i .',home_ca_bg_' . $i,
		...
		$fields[] = array(
			'type' => 'upload',
			'label' => __('Background Image', 'appic'),
			'name' => 'home_ca_bg_' .$i,
		);
		$fields[] = array(
			'type' => 'textbox',
		...
		$fields[] = array(
			'type' => 'upload',
			'label' => __('Background Image Hover', 'appic'),
			'name' => 'home_ca_bg_hover_' .$i,
			'dependency' => $dep,
		);
		$fields[] = array(
			'type' => 'textbox',
