...
// define('VP_URL', PARENT_URL . '/framework/vafpress');
...
if ( ! function_exists( 'theme_excerpt_more_link' ) ) {
	function theme_excerpt_more_link( $more = '...', $wrap = false )
	{
		static $moreText;
		if ( null === $moreText ) {
		...
		//if ( is_search() || get_theme_option('is_excerpt')) {
		if ($more === ' [&hellip;]') {
			//if ($moreText) return ' <a href="' . get_permalink() . '">' . $moreText . '</a>';
...
if ( ! function_exists( 'appic_the_content' ) ) {
	function appic_the_content($moreText = null)
	{
		if (get_theme_option('is_excerpt')) {
			the_excerpt();
		} else {
			// if (null === $moreText) {
			// 	$moreText = get_theme_option('excerpt_text');
...
// as this after save callback pass old values (bug in vaffpress) we will use before save event
//add_action('vp_option_set_after_save', 'action_on_theme_options_save');
add_action('vp_option_set_before_save', 'action_on_theme_options_save');
