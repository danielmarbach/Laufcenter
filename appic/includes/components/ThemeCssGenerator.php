...
class ThemeCssGenerator
{
	...
	public $defaultRequiredFontSubsets = array();
	...
	protected $requiredFontSubsets = array();
	...
	public function generateCss($themeSettings, $plainCss = false)
	{
		...
		if ($webfontsUrl = $this->getWebFontsApiUrlTags()) {
			if (!is_array($webfontsUrl)) {
				$webfontsUrl = array($webfontsUrl);
			}
			foreach ($webfontsUrl as $fontUrl) {
				$parts[] = '<link href="' . $fontUrl. '" rel="stylesheet" />';
			}
		}
	...
	public function resetRequiredFonts()
	{
		...
		$this->requiredFontSubsets = $this->defaultRequiredFontSubsets;
	}
	...
	public function getWebFontsApiUrlTags()
	{
		...
		$susetArg = '';
		if ($this->requiredFontSubsets) {
			$susetArg = '&subset=' . join(',', $this->requiredFontSubsets);
		}

		$result = array();

		if ($families) {
			$result[] = '//fonts.googleapis.com/css?family=' . join('|', $families) . $susetArg;
		}

		return apply_filters('theme-css-generator-font-urls', $result, $this->requiredFonts);
	}
	...
	public function registerRequiredFont($fontSettings)
	{
		if (!empty($fontSettings['font-face'])) {
			...
			$subset = !empty($fontSettings['subset']) ? $fontSettings['subset'] : '';
			if ($subset) {
				$fsId = $newFace . '_' . $subset;
				if (empty($this->requiredFontSubsets[$fsId])) {
					$this->requiredFontSubsets[$fsId] = $subset;
				}
			}
