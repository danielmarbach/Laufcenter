...
function tabs_shortcode($atts, $content = null)
{
	...
	$output = '<ul class="shortcode clear-list nav nav-tabs">';
	...
	for($i = 1; $i <= $numTabs; $i++) {
		...
		$tabIndex = $tabId . '_' . $i;
		$output .= '<li'.$class.'><a href="#tab'.$tabIndex.'" data-toggle="tab">'.$atts['tab'.$i].'</a></li>';
	}
	...
	foreach($atts as $key => $value) {
		...
		$find[] = '[/'.$key.']';
		$class = (1 == $i ? ' active' : '');
		$replace[] = '<div id="tab'.$tabIndex.'" class="tab-pane'.$class.'">';
		$replace[] = '</div><!-- .tab (end) -->';
		$i++;
	}
...
function tabs_vertical_shortcode($atts, $content = null)
{
	...
	$output = '<div class="shortcode tabbable tabs-left">' .
		'<ul class="clear-list nav nav-tabs">';
...
function td_shortcode($atts, $content = null)
{
	...
	$attributes = '';
	if ($atts['colspan'] > 1) {
		$attributes .= ' colspan="' . (int) $atts['colspan'] . '"';
	}
	if ($atts['rowspan'] > 1) {
		$attributes .= ' rowspan="' . (int) $atts['rowspan'] . '"';
	}
	if ($atts['class']) {
		$attributes .= ' class="' . esc_attr($atts['class']) . '"';
	}

	return '<td' . $attributes . '>' . do_shortcode($content) . '</td>';
}
